...
export default function AgentLayout({ children }) {
            ...
            <div className={`flex items-center gap-3 px-5 py-6 border-b border-white/10 ${collapsed ? 'justify-center px-3' : ''}`}>
                ...
                {!collapsed && (
                    <div>
                        ...
                        <p className="text-orange-300 text-[10px] font-medium mt-0.5">Agent Portal</p>
                    </div>
                )}
            </div>
                    ...
                    return (
                        <Link key={to} to={to} onClick={onNav} title={collapsed ? label : undefined}
                            className={`flex items-center gap-3 px-3 py-3 rounded-xl font-semibold text-sm
                                ...
                                ${active ? 'bg-white text-orange-900 shadow-md' : 'text-orange-100 hover:bg-white/10 hover:text-white'}
                                ${collapsed ? 'justify-center' : ''}`}>
                            <Icon className={`w-5 h-5 shrink-0 group-hover:scale-110 transition-transform ${active ? 'text-orange-900' : ''}`} />
                            {!collapsed && <span className="flex-1">{label}</span>}
                            {!collapsed && active && <ChevronRight className="w-4 h-4 text-orange-400" />}
                            {collapsed && (
                ...
                <div className={`flex items-center gap-3 px-3 py-3 ${collapsed ? 'justify-center' : ''}`}>
                    ...
                    {!collapsed && (
                        <div className="flex-1 min-w-0">
                            <p className="text-white text-sm font-semibold truncate">{user.full_name || 'Agent'}</p>
                            <p className="text-orange-300 text-[10px] truncate">{user.email || ''}</p>
                        </div>
                    )}
                </div>
                <button onClick={handleLogout} title={collapsed ? 'Logout' : undefined}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-orange-200 hover:bg-red-500/20 hover:text-red-300 transition-all text-sm font-medium group ${collapsed ? 'justify-center' : ''}`}>
    ...
    return (
        <div className="flex h-screen bg-gray-50 overflow-hidden">
            <aside className={`hidden lg:flex flex-col bg-gradient-to-b from-[#78350f] to-[#b45309]
            ...
            {mobileOpen && (
                <div className="fixed inset-0 z-50 lg:hidden">
                    ...
                    <aside className="absolute left-0 top-0 bottom-0 w-72 bg-gradient-to-b from-[#78350f] to-[#b45309] shadow-2xl flex flex-col">
                ...
                <header className="bg-white border-b border-gray-100 px-4 sm:px-6 h-16 flex items-center justify-between shrink-0 shadow-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-bold text-gray-900 leading-tight">
                                {navItems.find(n => isActive(n.to))?.label ?? 'Agent Portal'}
                            </h1>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-xl">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-600 to-amber-800 flex items-center justify-center text-white text-xs font-bold shadow-sm">
                                {initials}
                            </div>
