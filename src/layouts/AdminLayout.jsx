    ...
    const SidebarContent = ({ onNav }) => (
        <div className="flex flex-col h-full">
            ...
            <div className={`flex items-center gap-3 px-5 py-6 border-b border-white/10 ${collapsed ? 'justify-center px-3' : ''}`}>
                ...
                {!collapsed && (
                    <div>
                        ...
                        <p className="text-purple-300 text-[10px] font-medium mt-0.5">Admin Console</p>
                    </div>
                )}
            </div>
            ...
            <nav className="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
                {!collapsed && (
                    <p className="text-purple-300/70 text-[10px] font-bold uppercase tracking-widest px-3 mb-3">
                        Management
                    </p>
                )}
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-3 py-3 rounded-xl font-semibold text-sm
                                ...
                                ${active ? 'bg-white text-purple-900 shadow-md' : 'text-purple-100 hover:bg-white/10 hover:text-white'}
                                ${collapsed ? 'justify-center' : ''}`}
                        >
                            <Icon className={`w-5 h-5 shrink-0 transition-transform group-hover:scale-110 ${active ? 'text-purple-900' : ''}`} />
                            {!collapsed && <span className="flex-1">{label}</span>}
                            {!collapsed && active && <ChevronRight className="w-4 h-4 text-purple-400" />}
                            {collapsed && (
            ...
            <div className="border-t border-white/10 p-3 space-y-1">
                ...
                <button onClick={handleLogout} title={collapsed ? 'Logout' : undefined}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-purple-200 hover:bg-red-500/20 hover:text-red-300 transition-all text-sm font-medium group ${collapsed ? 'justify-center' : ''}`}>
